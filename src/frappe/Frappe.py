...
def main_process(fit_data,cl3_spt):
	...
	### Unpack the plain data dict (avoids serialising the full Fit object into Ray)
	...
	K_fixed_input        = fit_data['K_fixed_input']
	...
	#	FIT PARAMETERS
		...
		#		Recalculate K using the H_try derived before, then iterate to get again H until convergence (with max_iter = 100)
					K_old = K_try
					K_iter = float(np.squeeze(obs_cont_CLIIIScaling_dered-slab_cont_CLIIIScaling*H_try))
					if K_fixed is not None:
						K_iter = K_try#/cl3_cont_751
					H_iter = H_try
		...
		#		iteration to derive H again
					...
					iterations = 0
					if K_fixed is not None:
						# K is pinned: solve for H directly from the 360nm constraint
						H_iter = float(np.squeeze((obs_cont_360_dered - K_try*cl3_cont_360) / slab_cont_360))
					else:
		...
		#		store the best values
					K = float(np.squeeze(K_iter))
					H = float(np.squeeze(H_iter))
		# 		check that the normalization constants are > 0, otherwise it means that this model is not good
					if K < 0 or H <= 0:
						print( 'NOT POSSIBLE')
						continue
...
class Fit():
	def __init__(self,f_uvb = None,f_vis = None,f_nir = None,obj_in ='Unknown',dist=None ,dirOut = None,**kwargs):
		...
		for option, value in kwargs.items():
			...
			if option == 'spt':
				...
				print('A kwarg is not recognised')
				sys.exit(1)
		...
		# ------------------------------
		# 2b) STORE K_fixed IF PROVIDED
		# ------------------------------
		# If K_fixed is provided, K is pinned to this value in every main_process
		# iteration instead of being derived from the observed spectrum.
		self.K_fixed_input = K_fixed_input
		if K_fixed_input is not None:
			print(f'K fixed at {K_fixed_input:.4e}')

		# 3) BIG FOR
		# now makes use of the RAY package, RC
		time_init = time.time()
		ray.init(runtime_env={"working_dir":PATH+'/FrappeHelper/'}, log_to_driver=False)
		fit_data_ref = ray.put({
			'classIIIreadIn':       self.classIIIreadIn,
			'wl_UVB':               self.wl_UVB,
			'wl_VIS':               self.wl_VIS,
			'fl_UVB_in':            self.fl_UVB_in,
			'fl_VIS_in':            self.fl_VIS_in,
			'usedFeatures':         self.usedFeatures,
			'normWLandWidth':       self.normWLandWidth,
			'Av_list':              self.Av_list,
			'cl3_in_toSelectModel': self.cl3_in_toSelectModel,
			'K_fixed_input':        self.K_fixed_input,
		})
	...
	def resultsToFile(self):
		...
		#dist_cl3_pc_fin = dist_cl3_pc[np.where(name_cl3 == self.min_chi_sq_cl3)]

		#Lstar_cl3_fin = 10.**(logL_cl3[np.where(name_cl3 == self.min_chi_sq_cl3)])
		# read the sav file with the correctly sampled slab model
		s = readsav(PATH_SLAB_RESAMPLED_SAV+'slab_T%s_ne%s_tau%s_clIII_%s_UVB.sav' % (self.min_chi_sq_T,self.min_chi_sq_Ne,self.min_chi_sq_tau,self.cl3_in_toSelectModel))
		wl_slab_UVB_c,fl_slab_UVB_c = s['w'],s['f']
		s = readsav(PATH_SLAB_RESAMPLED_SAV+'slab_T%s_ne%s_tau%s_clIII_%s_VIS.sav' % (self.min_chi_sq_T,self.min_chi_sq_Ne,self.min_chi_sq_tau,self.cl3_in_toSelectModel))
		...
		f751_obs = np.median(fl_VIS[(self.wl_VIS>747)&(self.wl_VIS<755)])
		#####
		...
		if Teff < 3500:
			xlow = 730
			xhi =758
			flow = np.nanmedian(fl_VIS[(self.wl_VIS>xlow-0.4)&(self.wl_VIS<xlow+0.4)]) - (self.min_chi_sq_H*np.nanmedian(fl_slab[(wl_slab>xlow-0.4)&(wl_slab<xlow+0.4)]))
			fhigh = np.nanmedian(fl_VIS[(self.wl_VIS>xhi-0.4)&(self.wl_VIS<xhi+0.4)]) - (self.min_chi_sq_H*np.nanmedian(fl_slab[(wl_slab>xhi-0.4)&(wl_slab<xhi+0.4)]))
		...
		#f.write('SpT: %s \n' % (SpT_cl3[np.where(self.min_chi_sq_cl3 == name_cl3)]))
		f.write('Teff: %i \n' % (Teff))
		f.write('Chi2 = %0.3e \n' % self.min_chi_sq)
		f.write('\n')
	...
	def addEntryToTable(self, FileOut = None):
		...
		f751_obs = np.median(fl_VIS[(self.wl_VIS>747)&(self.wl_VIS<755)])
		#####
		#THIS bit is the interpolation over the VO feature (see HH14).
		wlScaling = 751
		...
		% (self.obj_in,self.dist_pc,SpTBestFit,float(self.min_chi_sq_cl3),Teff,self.min_chi_sq_Av,Rv,Lstar_input,np.log10(Lacc_Lsun),float(np.squeeze(mass_b15)),
	...
	def plotRegFit(self,close = False):
		...
		if plot_smooth == False:
			pl.plot(self.wl_UVB[self.ind_uvb],fl_UVB[self.ind_uvb],'k',zorder = 1)#,title=hdr['OBJECT'])#,xtitle='Wavelength [nm]',ytitle='Flux')
			...
			wl_UVB_smooth = self.wl_UVB[self.ind_uvb][::8]
		...
		pl.axis([330,470,-1e-16,3.5*np.mean(fl_UVB[(self.wl_UVB < 360) & (self.wl_UVB > 330)])])
		...
		# 1) normalize
		f_in_norm = np.nanmedian(fl_UVB[(self.wl_UVB > 417.5) & (self.wl_UVB < 419.5)])
		...
		# 2) plot
		pl.subplot(224)
		pl.plot(self.wl_UVB[self.ind_uvb],fl_UVB[self.ind_uvb]/f_in_norm,'k')#,title=hdr['OBJECT'])#,xtitle='Wavelength [nm]',ytitle='Flux')
		pl.xlabel('Wavelength [nm]')
		# pl.ylabel('Flux')
		...
		#TiO 710
		ind_vis = (self.wl_VIS >= min_wl_vis)
		pl.subplot(223)
		pl.plot(self.wl_VIS[ind_vis],fl_VIS[ind_vis],'r')#,title=hdr['OBJECT'])#,xtitle='Wavelength [nm]',ytitle='Flux')
		pl.xlabel('Wavelength [nm]')
		pl.ylabel('Flux')
		pl.axis([700,720,-1e-16,1.5*np.mean(fl_VIS[(self.wl_VIS < 720) & (self.wl_VIS > 700)])])
		...
		#pl.plot(wl_cl3_VIS[ind_vis_3],self.min_chi_sq_K*fl_cl3_VIS[ind_vis_3],'g')
	...
	def plotVeil(self,close =False):
		...
		eqw, err_eqw, mode, fwhm, lline = eqw_auto(self.wl_VIS,fl_VIS,670.78,size_cont=1.,plot='NO',mode='gauss',fwhm_out='Ja',lline_out='Ja')
		# eqw, err_eqw, mode, fwhm, lline = eqw_auto(self.wl_VIS,fl_VIS,656.28,size_cont=2.,plot='NO',mode='gauss',fwhm_out='Ja',lline_out='Ja')
		if nameWls == 'LM601':
			lline3 = 670.8
		elif nameWls == 'Sz94':
			lline3 = 670.3
		...
		##############
		# 1) normalize
		f_in_norm = np.mean(fl_VIS[(self.wl_VIS > 817.0) & (self.wl_VIS < 818.0)])
		...
		# 2) plot
		pl.subplot(221)
		pl.plot(self.wl_VIS[self.ind_vis]-shift,fl_VIS[self.ind_vis]/f_in_norm,'r')#,title=hdr['OBJECT'])#,xtitle='Wavelength [nm]',ytitle='Flux')
		...
		##############
		# 1) normalize
		f_in_norm = np.mean(fl_VIS[(self.wl_VIS > 767.50) & (self.wl_VIS < 769.0)])
		...
		##############
		# 1) normalize
		f_in_norm = np.mean(fl_VIS[(self.wl_VIS > 615.) & (self.wl_VIS < 615.8)])
		...
		##############
		# 1) normalize
		f_in_norm = np.mean(fl_VIS[(self.wl_VIS > 845.50) & (self.wl_VIS < 846.50)])
	...
	def plotPaschen(self,CLIII =False,close = False):
		...
		if CLIII == True:
			...
			##############
			pl.subplot(211)
			pl.plot(self.wl_UVB[self.ind_uvb],fl_UVB[self.ind_uvb],'r')#,title=hdr['OBJECT'])#,xtitle='Wavelength [nm]',ytitle='Flux')
			pl.title(self.obj_in)#+' - Chi$^2$= %0.3e' % (self.min_chi_sq))
			# pl.xlabel('Wavelength [nm]')
			pl.ylabel('Flux')
			pl.axis([460,550,-1e-16,3.5*np.mean(fl_UVB[(self.wl_UVB < 460) & (self.wl_UVB > 440)])])
			...
			##############
			pl.subplot(212)
			pl.plot(self.wl_VIS[self.ind_vis],fl_VIS[self.ind_vis],'r')#,title=hdr['OBJECT'])#,xtitle='Wavelength [nm]',ytitle='Flux')
			pl.xlabel('Wavelength [nm]')
			pl.ylabel('Flux')
			pl.axis([550,700,-1e-16,1.5*np.mean(fl_VIS[(self.wl_VIS < 650) & (self.wl_VIS > 600)])])
			pl.plot(wl_cl3_VIS[ind_vis_3],self.min_chi_sq_K*fl_cl3_VIS[ind_vis_3]+self.min_chi_sq_H*fl_slab_VIS_c[ind_vis_3],'b')
			...
			pl.axis([460,550,-1e-16,3.5*np.mean(fl_UVB[(self.wl_UVB < 460) & (self.wl_UVB > 440)])])
			#pl.plot(wl_cl3_UVB[ind_uvb_3],self.min_chi_sq_K*fl_cl3_UVB[ind_uvb_3]+self.min_chi_sq_H*fl_slab_UVB_c[ind_uvb_3],'b')
			#pl.plot(wl_cl3_UVB[ind_uvb_3],self.min_chi_sq_K*fl_cl3_UVB[ind_uvb_3],'g')
			pl.plot(wl_cl3_UVB[ind_uvb_3],self.min_chi_sq_H*fl_slab_UVB_c[ind_uvb_3],'c')
			pl.errorbar(wlFeat,self.min_chi_sq_K*features,self.min_chi_sq_K*errors,xerr = Xrange,fmt='.',c='g',zorder = 10)
			pl.errorbar(wlFeat,fit_cont,yerr=fit_std,xerr= Xrange,fmt='.',c='b',zorder = 10)
